...
def authorize_request(
    ...
    try:
        ...
        # Running outside Riff/Databutton platform — use a minimal config fallback
        from ..config import Config
        cfg = Config(ENVIRONMENT=os.environ.get("ENV", "development"))
...
def get_auth_configs(request: HTTPConnection) -> list[AuthConfig]:
    # Try databutton_app_state first (Riff/Databutton platform), then fall back to direct state
    databutton_state = getattr(request.app.state, "databutton_app_state", None)
    if databutton_state is not None:
        auth_configs: list[AuthConfig] = getattr(databutton_state, "auth_configs", None) or []
    else:
        auth_configs = getattr(request.app.state, "auth_configs", None) or []
    return auth_configs
...
def get_audit_log(request: HTTPConnection) -> Callable[[str], None] | None:
    databutton_state = getattr(request.app.state, "databutton_app_state", None)
    if databutton_state is not None:
        return getattr(databutton_state, "audit_log", None)
    return None
