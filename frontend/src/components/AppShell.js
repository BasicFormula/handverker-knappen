import { BriefcaseBusiness, CircleHelp, HardHat, LayoutDashboard, MailPlus, Plus, UserRound } from "lucide-react";
...
const links = [
  ...
  { to: "/lansering", label: "Lansering", icon: MailPlus },
    ...
    <div className="app-frame" data-testid="app-shell">
      <aside className="sidebar" data-testid="main-navigation">
        ...
        <nav className="nav-links" aria-label="Hovednavigasjon">
          {links.map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} end={to === "/"} className="nav-link" data-testid={`nav-${label.toLowerCase().split(" ").join("-")}`}><Icon size={18} strokeWidth={1.8} />{label}</NavLink>)}
        </nav>
      ...
      <main className="main-content"><header className="topbar"><div className="mobile-brand" data-testid="mobile-brand">håndverker<span>knappen</span></div><div className="topbar-actions"><div className="trust-note" data-testid="bankid-protection-note"><BankIdBadge /> Alle er verifisert</div><Link className="primary-button" to="/oppdrag/ny" data-testid="topbar-create-job-button"><Plus size={18} /> Legg ut oppdrag</Link></div></header><nav className="mobile-nav" aria-label="Mobilnavigasjon" data-testid="mobile-navigation">{links.map(({ to, label, icon: Icon }) => <NavLink key={to} to={to} end={to === "/"} className="mobile-nav-link" data-testid={`mobile-nav-${label.toLowerCase().split(" ").join("-")}`}><Icon size={16} />{label}</NavLink>)}</nav><Outlet /></main>
    </div>
  );
}