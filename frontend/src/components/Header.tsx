    ...
    <header className="sticky top-0 left-0 right-0 glass-surface-light backdrop-blur-xl z-50 border-b border-border bar-gradient-bottom">
      <div className="container mx-auto px-4 h-20 sm:h-24 md:h-28 lg:h-32 flex justify-between items-center">
        <Link to="/" className="flex items-center gap-3 group">
          <span className="font-headline text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-[-0.01em] leading-none text-forest-900 group-hover:opacity-90 transition-all origin-left">
            Håndverker<span className="text-amber-500">Knappen</span>
          </span>
        </Link>
        <nav className="hidden md:flex gap-4 items-center">
          <Button asChild variant="ghost" className="h-11 px-5 rounded-xl font-semibold tracking-[0.02em]">
            <Link to="/for-handverkere-page" className="text-slate-600 hover:text-amber-600 transition-colors">
              {t('nav_for_craftsmen')}
            </Link>
          </Button>
          <Button asChild variant="ghost" className="h-11 px-5 rounded-xl font-semibold tracking-[0.02em]">
            <Link to="/find-craftsmen" className="text-slate-600 hover:text-amber-600 transition-colors">
              {t('nav_market')}
            </Link>
          </Button>
          <Button asChild variant="ghost" className="h-11 px-5 rounded-xl font-semibold tracking-[0.02em]">
            <Link to="/contact-us-page" className="text-slate-600 hover:text-amber-600 transition-colors">
              {t('nav_contact')}
            </Link>
          </Button>
        </nav>
