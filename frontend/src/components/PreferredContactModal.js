  ...
  return (
    <div className="modal-backdrop" role="presentation" data-testid="preferred-contact-modal">
      <section className="payment-modal" role="dialog" aria-modal="true" aria-label="Foretrukket håndverker">
        ...
        <Send size={28} />
        <p className="eyebrow">FORETRUKKET HÅNDVERKER</p>
        <h2>{person.name}</h2>
        <p>Vi sender en prioritert forespørsel. Hvis håndverkeren ikke gir positivt svar innen 48 timer, refunderes 200 kr automatisk.</p>
        ...
        <div className="fee-row" data-testid="preferred-contact-fee"><span><ShieldCheck size={16} /> 48-timers svarregel</span><strong>200 kr</strong></div>
      </section>
    </div>
  );
}